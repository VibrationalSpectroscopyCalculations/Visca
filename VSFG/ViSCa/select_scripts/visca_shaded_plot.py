...
sfg_pol_combs_dir_files = os.listdir(path=sfg_pol_combs_dir)
RSS_list = []
for i,f,RSS in zip(RSS_data['initial_frame'],RSS_data['final_frame'],RSS_data['RSS']):
    if RSS>RSS_cutoff:
        continue
    spec_file = 'pol_comb_sfgf%ito%i.txt'%(int(i),int(f))
    spec_files.append(spec_file)
    RSS_list.append(RSS)
    if spec_file in sfg_pol_combs_dir_files:
...
#        visca.Write_plot(settings_file, str(int(i)), str(int(f)))
    if RSS==lowest_RSS:
       ...
       best_spec = visca.Read_pol_comb_file(sfg_pol_combs_dir+'/'+spec_file, settings['Normalize_wrt'])
...
for spec_file in spec_files:
    spec_data = visca.Read_pol_comb_file(sfg_pol_combs_dir+'/'+spec_file, settings['Normalize_wrt'])
    if first_flag:
