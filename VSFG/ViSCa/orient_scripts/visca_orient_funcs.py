...
def Write_plot(settings_file, theta, psi):
    ...
    os.chdir(plot_dir)
#    command = settings['vmd_command']+' -dispdev text -e '+settings['tcl_scripts_location']+'"splitter_i_to_f.tcl" -args "%s%s.dcd" %s %s > ../VMD_log.txt'%(settings['dcd_location'],settings['name'],str(theta),str(psi))
    command = settings['vmd_command']+' -dispdev text -e "'+settings['tcl_scripts_location']+'rotator_theta_phi_psi.tcl" -args "%s.pdb" %i %i %i > ../VMD_log.txt'%(settings['name'], int(theta), int(phi), int(psi))
    os.system(command)
