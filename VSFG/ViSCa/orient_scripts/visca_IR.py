...
try:
    work_dir = 'Working_Directory_'+settings['name']+'/IR'
except KeyError:
...
command = 'time "'+fortran_script+'" -pdb '+settings['name']+'.pdb -spec_min 1500 -spec_max 1800 -IR -inhom 0 -width %s -coup 1 -dip 0 -nncm 1 -avgOH 1 -mOH %s -Omega0 %s > fortran_log.txt 2> /dev/null'%(settings['width'],settings['mOH'],settings['Omega0'])
print('Fortran command:',command)
os.system(command)

IR = np.loadtxt(settings['name']+'_IR.txt')
wavenumber = IR[:,0]
ir_intensity = IR[:,1]
norm_ir_intensity = ir_intensity / max(ir_intensity)

plt.plot(wavenumber,norm_ir_intensity)
fs = 12
font = {'family' : 'Arial',
        'weight' : 'normal',
        'size'   : fs}
...
plt.title(settings['name'][0:4]+" IR spectrum")
plt.xlabel("Frequency",fontsize=fs)
plt.ylabel("Normalised IR Intensity",fontsize=fs)
plt.xlim([1500, 1800])
plt.ylim([-0.1, 1.1])
