...
if 'post_clean' not in settings.keys() or settings['post_clean']=='True':
    ...
    os.system(f'printf "All .dcd and .txt files was removed from this folder after running {sys.argv[0]}\n" >> {dcd_dir}/this_folder_was_cleaned.txt')
    os.system(f'printf "Deleted:\n" >> {dcd_dir}/this_folder_was_cleaned.txt')
    os.system(f'ls -ah {dcd_dir}/f*.dcd >> {dcd_dir}/this_folder_was_cleaned.txt')
    os.system(f'printf "\n" >> {dcd_dir}/this_folder_was_cleaned.txt')
    os.system(f'ls -ah {dcd_dir}/f*_SFG.txt >> {dcd_dir}/this_folder_was_cleaned.txt')
    os.system(f'printf "\n\n" >> {dcd_dir}/this_folder_was_cleaned.txt')

    os.system(f'rm {dcd_dir}/f*.dcd')
    os.system(f'rm {dcd_dir}/f*_SFG.txt')
    ...
    os.system(f'printf "All .txt files was removed from this folder after running {sys.argv[0]}\n" >> {sfg_pol_comb_path}/this_folder_was_cleaned.txt')
    os.system(f'printf "Deleted:\n" >> {sfg_pol_comb_path}/this_folder_was_cleaned.txt')
    os.system(f'ls -ah {sfg_pol_comb_path}/pol_comb_sfg*.txt >> {sfg_pol_comb_path}/this_folder_was_cleaned.txt')
    os.system(f'printf "\n\n" >> {sfg_pol_comb_path}/this_folder_was_cleaned.txt')

    os.system(f'rm {sfg_pol_comb_path}/pol_comb_sfg*.txt')
else:
    input('Warning: You are potentially storing many dcd files because ViSCa post clean is turned off. <Enter to continue>')
