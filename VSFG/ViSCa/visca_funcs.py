...
def RSS(x_calc, y_calc, x_exp, y_exp, plot=False):
    ...
    return sum([RS(x_e, y_e) for x_e,y_e in zip(x_exp,y_exp)])
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    print("Non-Read line found in input file:",line)
                    continue
                if key[-8:]=="location":
...
def Write_plot(settings_file, fi, ff):
    ...
    main_dir = os.getcwd()
    working_dir = main_dir+'/Working_Directory_'+settings['name']
    ...
    calc_data = {'xvals':sfg_data[:,0], 'SSP': sfg_data[:,1], 'PPP': sfg_data[:,2], 'SPS': sfg_data[:,3], 'PSS': sfg_data[:,4]}
    os.chdir('../..')
